    ...
    <div style={{ fontFamily: "'Barlow Condensed', sans-serif", background: '#020606', minHeight: '100vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'stretch' }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .grid-bg::after { content: ''; position: absolute; inset: 0; background: radial-gradient(ellipse 70% 60% at 80% 40%, rgba(0,212,182,0.07) 0%, transparent 65%); }
        @keyframes fadeUp { from { opacity: 0; transform: translateY(22px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes pulse-ring { 0% { transform: scale(1); opacity: 0.7; } 100% { transform: scale(2.2); opacity: 0; } }
        @keyframes shimmer { 0% { transform: translateX(-100%) skewX(-15deg); } 100% { transform: translateX(400%) skewX(-15deg); } }
        @keyframes float { 0%, 100% { transform: translateY(0px) rotate(0deg); } 50% { transform: translateY(-12px) rotate(1deg); } }
        ...
        .input-wrap input { flex: 1; background: transparent; border: none; outline: none; font-family: 'Barlow', sans-serif; font-size: 14px; color: #fff; padding: 14px 18px; letter-spacing: 0.02em; caret-color: #00d4b6; }
        .input-wrap input::placeholder { color: rgba(255,255,255,0.2); }
        .input-label { font-family: 'Barlow', sans-serif; font-size: 10px; font-weight: 700; letter-spacing: 0.28em; text-transform: uppercase; color: rgba(255,255,255,0.3); margin-bottom: 7px; display: block; transition: color 0.25s; }
        .input-label.active { color: #00d4b6; }
        ...
        .btn-primary .shimmer-bar { animation: shimmer 2.5s ease-in-out infinite; position: absolute; top: 0; left: 0; width: 30%; height: 100%; background: rgba(255,255,255,0.25); }
        .btn-google { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); color: rgba(255,255,255,0.7); font-family: 'Barlow', sans-serif; font-size: 13px; font-weight: 600; letter-spacing: 0.05em; padding: 14px 24px; cursor: pointer; width: 100%; display: flex; align-items: center; justify-content: center; gap: 12px; transition: border-color 0.25s, background 0.25s, color 0.25s; }
        .btn-google:hover { border-color: rgba(0,212,182,0.35); background: rgba(0,212,182,0.04); color: #fff; }
        .divider { display: flex; align-items: center; gap: 16px; }
        .divider-line { flex: 1; height: 1px; background: rgba(255,255,255,0.07); }
        .divider-text { font-family: 'Barlow', sans-serif; font-size: 10px; font-weight: 600; letter-spacing: 0.25em; text-transform: uppercase; color: rgba(255,255,255,0.22); }
        .perk-check { width: 20px; height: 20px; border-radius: 50%; background: rgba(0,212,182,0.12); border: 1px solid rgba(0,212,182,0.25); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .checkbox-wrap { display: flex; align-items: flex-start; gap: 12px; cursor: pointer; }
        .checkbox-box { width: 18px; height: 18px; border: 1px solid rgba(255,255,255,0.2); background: transparent; flex-shrink: 0; margin-top: 1px; display: flex; align-items: center; justify-content: center; transition: border-color 0.25s, background 0.25s; cursor: pointer; }
        .checkbox-box.checked { border-color: #00d4b6; background: rgba(0,212,182,0.15); }
        .link-accent { color: #00d4b6; text-decoration: none; font-weight: 600; transition: opacity 0.2s; }
        .link-accent:hover { opacity: 0.75; }
        .ghost-text { position: absolute; font-size: clamp(80px, 12vw, 180px); font-weight: 900; letter-spacing: -0.04em; color: transparent; -webkit-text-stroke: 1px rgba(0,212,182,0.04); text-transform: uppercase; user-select: none; pointer-events: none; line-height: 1; }
        .error-box { background: rgba(255,60,60,0.08); border: 1px solid rgba(255,60,60,0.25); padding: 12px 16px; margin-bottom: 16px; font-family: 'Barlow', sans-serif; font-size: 12px; color: rgba(255,100,100,0.9); letter-spacing: 0.04em; }
      `}</style>
      ...
      <div style={{ flex: 1, position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 64px', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ width: '100%', maxWidth: 440 }}>
          <Link href="/" style={{ display: 'inline-block', fontFamily: "'Barlow Condensed', sans-serif", fontSize: 26, fontWeight: 900, letterSpacing: '-0.02em', color: '#fff', textDecoration: 'none', marginBottom: 36 }} className="a1">
            mdfld<span style={{ color: '#00d4b6' }}>.</span>
          </Link>
          ...
          <div className="a5" style={{ marginBottom: 14 }}>
            ...
            <div className={`input-wrap${focus === 'password' ? ' focused' : ''}`}>
              <input type={showPass ? 'text' : 'password'} placeholder="Min. 8 characters" value={form.password} onChange={set('password')} onFocus={() => setFocus('password')} onBlur={() => setFocus(null)} />
              <button onClick={() => setShowPass(p => !p)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.28)', padding: '0 16px', display: 'flex', alignItems: 'center' }}
                onMouseEnter={e => (e.currentTarget.style.color = '#00d4b6')} onMouseLeave={e => (e.currentTarget.style.color = 'rgba(255,255,255,0.28)')}>
                {showPass ? <EyeOff size={15} /> : <Eye size={15} />}
              </button>
            </div>
          </div>
          ...
          <div className="a5" style={{ marginBottom: 22 }}>
            ...
            <div className={`input-wrap${focus === 'confirm' ? ' focused' : ''}`}>
              <input type={showConfirm ? 'text' : 'password'} placeholder="Repeat password" value={form.confirm} onChange={set('confirm')} onFocus={() => setFocus('confirm')} onBlur={() => setFocus(null)} />
              <button onClick={() => setShowConfirm(p => !p)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.28)', padding: '0 16px', display: 'flex', alignItems: 'center' }}
                onMouseEnter={e => (e.currentTarget.style.color = '#00d4b6')} onMouseLeave={e => (e.currentTarget.style.color = 'rgba(255,255,255,0.28)')}>
                {showConfirm ? <EyeOff size={15} /> : <Eye size={15} />}
              </button>
            </div>
      ...
      <div style={{ flex: '0 0 40%', position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '48px 56px' }}>
        ...
        <div className="a3" style={{ height: 2, width: 140, background: 'linear-gradient(90deg, #00d4b6, transparent)', boxShadow: '0 0 8px #00d4b6', marginBottom: 36 }} />
        <div className="a4" style={{ display: 'flex', flexDirection: 'column', marginBottom: 40 }}>
          {PERKS.map((perk, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div className="perk-check"><Check size={10} color="#00d4b6" strokeWidth={3} /></div>
              <span style={{ fontFamily: "'Barlow', sans-serif", fontSize: 13, fontWeight: 400, color: 'rgba(255,255,255,0.6)', letterSpacing: '0.02em' }}>{perk}</span>
            </div>
          ))}
        </div>
